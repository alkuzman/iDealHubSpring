...
public interface NoticeRepository extends GraphRepository<Notice> {
    ...
    @Query("MATCH (n:AbstractNotice)-[r:RECIPIENT]->(recipient:Agent) WHERE recipient.email = { recipient_email } AND NOT exists(n.seen) WITH COUNT(n) as count RETURN count")
    Integer getCount(@Param("recipient_email") String email);

    @Query("MATCH (n:AbstractNotice)-[r:RECIPIENT]->(recipient:Agent) WHERE recipient.email = { recipient_email } AND NOT exists(n.seen) SET n.seen = { seen_date } WITH COUNT(n) as count RETURN count ")
    void markAsSeen(@Param("recipient_email") String email, @Param("seen_date") String now);

    @Query("MATCH (n:AbstractNotice)-[r:RECIPIENT]->(recipient:Agent) WHERE recipient.email = { recipient_email } AND NOT exists(n.seen) RETURN n")
