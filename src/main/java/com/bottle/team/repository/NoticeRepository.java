...
public interface NoticeRepository extends GraphRepository<Notice> {
    @Query("MATCH (n:Notice)-[r:RECIPIENT]->(recipient:Agent) WHERE recipient.email = { recipient_email } WITH n SKIP { offset } LIMIT { limit } MATCH p=(n)-[*0..]->(m) RETURN n, p ORDER BY n.creationDate DESC")
